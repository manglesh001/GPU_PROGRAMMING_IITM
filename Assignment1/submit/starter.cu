...
__global__ void CalculateInvertedGrayScale(long int *d_red, long int *d_green,long int *d_blue,long int *T1,int m,int n)
{
    unsigned id=blockIdx.x * blockDim.x +threadIdx.x;
    if(id<m*n){
    long int i=id/n;
    long int j=id%n;
    long int gray=(d_red[id]+ d_green[id] +d_blue[id])/3;
    T1[(m-1-i)*n +j]=gray;  
}}

__global__ void CalculateThomasTransformation(long int *red, long int *green,long int *blue,long int *T2,int m,int n)
{
    unsigned idx=blockIdx.x * blockDim.x + threadIdx.x;
    if(idx < m*n)
{
    T2[idx] = (long int)((red[idx] * 0.5 ) +(long int)sqrt((float)green[idx]) + blue[idx]);
